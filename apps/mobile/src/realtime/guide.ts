...
- get_next_route_candidates 결과의 routes에는 아직 안내되지 않은 후보만 들어 있다. routes가 있으면 그 후보만 안내하고, 이미 안내한 후보를 다시 말하지 않는다.
- get_next_route_candidates 결과의 routes가 비어 있고 exhausted가 true이면 더 이상 안내할 다른 후보가 없다고 말하고, 이전 후보를 반복하거나 search_routes를 임의로 다시 호출하지 않는다.
