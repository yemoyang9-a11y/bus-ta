...
test("provider reads process env per request and scope does not change MOCK selection", (t) => {
  const mode = process.env.ROUTE_SEARCH_MODE;
  const scope = process.env.ROUTE_SEARCH_SCOPE;
  t.after(() => {
    if (mode === undefined) delete process.env.ROUTE_SEARCH_MODE; else process.env.ROUTE_SEARCH_MODE = mode;
    if (scope === undefined) delete process.env.ROUTE_SEARCH_SCOPE; else process.env.ROUTE_SEARCH_SCOPE = scope;
  });
  delete process.env.ROUTE_SEARCH_MODE;
  const real = getRouteSearchProvider();
  process.env.ROUTE_SEARCH_MODE = "MOCK";
  process.env.ROUTE_SEARCH_SCOPE = "MULTIMODAL";
  const mock = getRouteSearchProvider();
  assert.notEqual(mock, real);
  delete process.env.ROUTE_SEARCH_SCOPE;
  assert.equal(getRouteSearchProvider(), mock);
  process.env.ROUTE_SEARCH_MODE = "REAL";
  assert.equal(getRouteSearchProvider(), real);
});
