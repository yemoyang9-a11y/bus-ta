...
// ─────────────────────────────────────────────
// 후보 선택 전 도착 예정 시간 안내 (2026-08-26)
//
// 사용자는 "몇 분 뒤에 오는지"를 알아야 노선을 고를 수 있는데, 기존에는 도착정보를
// create_trip 에서만 조회해서 선택 시점에는 알 방법이 없었다. 안내할 후보 2개에
// 한해 검색 단계에서 함께 조회한다.
// ─────────────────────────────────────────────
...
test("안내할 후보에 도착 예정 시간을 함께 실어 보낸다", async () => {
  const result = await searchRoutes(validRequest, {
    searchRoutes: mockSearchRoutes,
    generateRouteGuide: async () => ({
      selectedCandidates: [{ candidateId: 1, guideMessage: "1번 후보 안내입니다." }],
    }),
    getArrivals: async () => [arrival(6), arrival(18)],
  ...
  if (result.httpStatus !== 200) return;
  assert.deepEqual(
    result.body.routes[0]?.arrivals?.map((a: { predictedArrivalMinutes: number }) => a.predictedArrivalMinutes),
    [6, 18],
  );
});

test("같은 정류장·노선 후보는 도착정보를 한 번만 조회한다", async () => {
  const baseRoutes = await mockSearchRoutes(validRequest);
  const shared: Route[] = [
  ...
  assert.equal(calls, 1, "같은 정류장·노선이면 GBIS 를 한 번만 불러야 한다");
});

test("도착정보 조회에 실패해도 노선 안내 자체는 유지한다", async () => {
  const result = await searchRoutes(validRequest, {
    searchRoutes: mockSearchRoutes,
    generateRouteGuide: async () => ({
      selectedCandidates: [{ candidateId: 1, guideMessage: "1번 후보 안내입니다." }],
  ...
  assert.equal(result.httpStatus, 200);
  if (result.httpStatus !== 200) return;
  assert.equal(result.body.routes.length, 1, "도착 시간을 못 얻었다고 노선을 빼면 안 된다");
  assert.equal(result.body.routes[0]?.arrivals, undefined);
  assert.equal(result.body.routes[0]?.guideMessage, "1번 후보 안내입니다.");
});

test("도착정보 조회기를 주입하지 않으면 기존 계약 그대로 arrivals 없이 응답한다", async () => {
  const result = await searchRoutes(validRequest, {
    searchRoutes: mockSearchRoutes,
    generateRouteGuide: async () => ({
      selectedCandidates: [{ candidateId: 1, guideMessage: "1번 후보 안내입니다." }],
