 ...
 * 남는 노선은 대개 이 상한보다 적다(캡처된 수원대->병점은 4개).
 *
 * 예모님 확정(2026-09-03): 후보 개수 정책은 5개로 통일한다. 앱이 후보를 5분간
 * 보관하고 2개씩 안내하므로, 그보다 많이 돌려줘도 쓰이지 않는다.
 * search-routes.service.ts 가 같은 값을 명시적으로 넘기므로 검색 응답 동작은
 * 이 기본값과 무관하지만, 두 값이 어긋나면 정책이 어디 기준인지 헷갈린다.
 */
