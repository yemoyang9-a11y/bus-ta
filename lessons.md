...
## 2026-09-05 (2) — 발음을 데이터로 내려도, 원본이 옆에 있으면 모델은 원본을 읽는다

**증상.** 앞 항목에서 `routeNoSpoken`을 Function 결과에 실어 보내고 "그대로 읽어라"라고
지시하는 방식으로 고쳤다고 기록했는데, 실기기에서 여전히 틀렸다. AI가 `M4101`을
"엠 사천 일공일"처럼 말했다. 우리 변환 함수는 "엠 사 일 공 일"을 만들므로 그 소리가
나올 수 없다 — 모델이 우리 값을 쓰지 않았다는 증거다.

**원인.** payload를 직접 찍어 보니 이랬다.

```json
{ "routeNo": "M4101", "routeNoSpoken": "엠 사 일 공 일" }
```

모델 눈앞에 원본 표기가 그대로 놓여 있었다. 음성 모델은 문장을 만들면서 그걸 자기
방식으로 발음한다. **"이 필드 말고 저 필드를 읽어라"는 결국 또 하나의 지시였다.**
프롬프트 의존을 벗어났다고 생각했지만 한 겹만 벗어난 것이었다.

**검증된 해결책.** 모델 payload에서 원본 `routeNo`를 아예 삭제하고 `routeNoSpoken`만
남겼다. 보지 못한 문자열은 발음할 수 없다. 모델은 원본이 없어도 되는데, 노선 선택은
`candidateId`로 이뤄지고 `create_trip` 요청 본문은 Dispatcher가 앱 상태의 실제 후보에서
다시 만들기 때문이다. 같은 이유로 `create_trip` tool 스키마도 `destination`·`candidateId`
둘만 받도록 줄였다 — 나머지는 어차피 무시되던 값이라, 남겨 두면 모델이 없는 값을
지어내게만 한다. 앱 상태와 서버 응답에는 실제 `routeNo`가 그대로 남는다(화면 표시와
요청 본문이 쓴다). 서버 테스트 327/327 통과, typecheck 3/3, build 통과.

**교훈 일반화**
- 모델에게 "A 대신 B를 써라"라고 시키는 설계는 A가 여전히 보이는 한 지시일 뿐이다.
  강제하려면 **A를 안 보이게** 해야 한다. 선택지를 없애는 것과 선택을 지시하는 것은
  신뢰도가 다르다.
- 프롬프트를 고친 뒤 "이제 데이터로 확정했다"고 결론짓기 전에, **모델이 실제로 받는
  payload를 한 번 찍어 본다.** 이번에는 그걸 찍어 보고서야 원본이 함께 가고 있다는 걸 알았다.
- 같은 증상을 세 번째 고치고 있다면 접근이 아니라 **가정**을 의심한다. 앞 두 번은
  "모델이 규칙을 따를 것"이라는 같은 가정 위에 있었다.
- 모델에게 넘기는 tool 스키마에 "어차피 무시하는 필드"를 남겨 두지 않는다. 모델은
  그 칸을 채우려고 값을 지어낸다.
