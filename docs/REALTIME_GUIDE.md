...
- 노선 번호 발음: 모델이 발음을 계산하지 않는다. 앱이 `@bus-ta/shared`의 `toSpokenRouteNo()`로 만든 `routeNoSpoken`을 Function 결과에 실어 보내고, 모델은 그 문자열을 그대로 읽은 뒤 "번"을 붙인다. 실제 `routeNo` 데이터는 변경하지 않으며, `routeNoSpoken`은 모델에게 보내는 payload에만 있고 서버 응답이나 공개 API 계약에는 없다.

  변환 규칙은 기존 계약과 같다. 하이픈(`-`)을 기준으로 나누고 하이픈 양쪽 숫자를 이어 붙여 자릿수를 세지 않는다. 각 숫자 덩어리가 네 자리 이상이면 한 자리씩(`1551` → "일 오 오 일"), 세 자리 이하면 일반적인 한국어 수 읽기로 읽는다(`205` → "이백오", `35` → "삼십오"). 하이픈은 "다시"로 읽고(`700-2` → "칠백 다시 이"), 알파벳과 괄호 안 표시를 생략하지 않는다(`1551B` → "일 오 오 일 비", `35-2(A)` → "삼십오 다시 이 에이").

  **프롬프트 규칙에서 데이터로 옮긴 이유.** 2026-09-05 시연에서 AI가 `35`를 "셋다섯", `15-2`를 "일번", `82-1`을 "팔십이번"으로 말했다. 뒤 숫자가 통째로 빠지면 시각장애인 사용자는 다른 버스를 탄다. 같은 규칙을 프롬프트에 세 차례 조여 넣었는데도 계속 틀렸으므로, 지시 따르기에 의존하지 않고 순수 함수로 확정해 테스트로 고정한다(`apps/server/src/integration/route-number-speech.test.ts`).

  사용자 발화를 **알아듣는** 규칙은 그대로 유지한다. 음성 인식에서 하이픈이 "다시"·"대시"·"하이픈"으로 바뀌거나 생략될 수 있으므로 가능한 숫자-숫자 형태를 함께 고려하되, 실제 `search_routes` 결과의 `routeNo`와 일치할 때만 후보 선택에 쓴다.
