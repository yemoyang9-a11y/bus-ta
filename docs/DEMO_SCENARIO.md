...
외부 경로 API 없이 전체 연결 흐름을 확인할 때는 서버 `.env`에
`ROUTE_SEARCH_MODE=MOCK`를 설정한다. 그러면 사용자가 어떤 목적지를 말해도
시연 노선 `1551`이 반환되고, 이후 비콘 조회는 `1551 → BUS_1551_001` 매칭을 사용한다.
...
1. 앱이 `GET /api/beacons?routeNo=1551`을 호출합니다.
2. 백엔드는 시연용 비콘 ID를 반환합니다.
3. 예시 값은 `BUS_1551_001`입니다.
