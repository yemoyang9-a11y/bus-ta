...
5. 앱은 위치 구독의 새 표본으로 `PATCH /api/trips/{tripId}/status`를 호출한다. 현재 요청한 위치 간격은 2초이며, 실제 수신 간격은 기기·OS에 따라 달라지고 이전 요청이 진행 중이면 다음 표본은 건너뛴다. 탑승확정 전에는 GPS가 들어와도 `WAITING_BUS`이며 하차벨을 만들지 않고, 확정 후에만 백엔드가 진행 상태와 하차벨 생성 여부를 결정한다.
