...
- 문서의 계약과 현재 구현 사실은 구분한다. 현재 사실은 `yemo-develop` 코드, `packages/shared`, 현재 테스트 및 실제 Supabase 적용 상태로 확인한다.
- `POST /api/realtime/session` 구현은 `yemo-develop` 코드와 테스트에서 확인됐지만, 실제 OpenAI 연결과 배포 환경 동작 검증은 별도 확인 대상이다.
