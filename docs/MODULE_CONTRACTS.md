...
탑승이 확정되면 비콘 스캔을 멈춘 뒤 스마트지팡이 BLE 연결도 끊는다. 승차 안내(버스 접근 진동)가 끝나면 지팡이를 붙들고 있을 이유가 없고, 연결이 남으면 지팡이 배터리를 계속 쓴다. 하차 안내는 하차벨 보드가 맡으므로 그 연결은 유지한다.

**순서를 반드시 지킨다.** 비콘 스캔 시작·중지는 지팡이에 BLE 명령을 써서 동작하므로, 연결을 먼저 끊으면 중지 명령이 전달되지 않아 탑승 뒤에도 지팡이가 계속 진동한다. 중지가 실제로 성공한 뒤에만 끊고, 상한까지 실패하면 끊지 않고 연결을 남긴다 — 끊어 버리면 중지 명령을 다시 보낼 길이 사라진다. 배터리보다 진동이 멈추지 않는 쪽이 나쁘다.
