...
아래의 중간평가 당시 후보 2개·폴더 구조·mock 설명은 역사적 설계다. 현재는 서버가 최대 5개 후보를 보존하고 상위 2개에 안내 문장을 붙인다. 탑승확정 성공 전에는 GPS만으로 `ON_BUS`가 되지 않는다. 실제 구현과 최신 변경 검증은 [2026-09-22 결과](superpowers/plans/2026-09-22-rehearsal-results.md)를 따른다.
...
- `bus_beacons`: 노선·차량과 비콘 ID 매칭. 실DB 조회 및 실제 비콘 ID 정렬 migration이 존재한다. 현재 설치 보드와 운영 DB 값의 일치는 시연 전 확인한다.
...
7/1 중간평가에서는 실제 BLE 하드웨어 연동을 필수 구현 범위에서 제외하고 mock 비콘과 mock 하차벨로 연결 구조를 검증했습니다. 이후 실제 비콘 ID 및 BLE 명령·Notify 연동 코드가 추가됐습니다. 2026-09-22 전달된 리허설의 근접 진동·수동 탑승·물리 벨 관측과 최신 수정의 전체 검증은 구분하며, 현재 재검증은 [실행 계획](superpowers/plans/2026-09-22-rehearsal-backlog.md)과 [현장 시험표](DEMO_SCENARIO.md)를 따릅니다.
