...
이 문서는 초기 기능 요구사항과 당시 상태 표현을 보존합니다. 현재 코드에는 검색·Realtime·BLE·탑승확정·하차벨 연동이 있으며 아래의 옛 `확인 필요`만으로 미구현을 판단하지 않습니다. 현재 구현·검증·외부 확인 상태는 [2026-09-22 실행 결과](superpowers/plans/2026-09-22-rehearsal-results.md)를 우선합니다. 이번 조사에서는 최신 Notion 원문 접근이 되지 않아 Notion과의 재동기화 완료를 주장하지 않습니다.
