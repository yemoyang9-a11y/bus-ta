...
리허설에서 관측된 지팡이 재접근 진동 미복구와 하차 이후 종료 안내 누락, 과거 보류 항목은 [실행 계획](superpowers/plans/2026-09-22-rehearsal-backlog.md)에 묶어 진행한다. 실제 실행·시험·미확인 항목은 [결과 기록](superpowers/plans/2026-09-22-rehearsal-results.md)을 기준으로 한다. 아래 평가 단계별 목록은 개발 당시 범위이며 빈 항목을 현재 미구현으로 해석하지 않는다.
