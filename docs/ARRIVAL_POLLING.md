...
"버스 놓쳤어요"처럼 사용자가 최신 값을 명시적으로 요구한 경우에는 갱신 시점 전이라도 다시 조회한다.
`ArrivalCache.get(target, { refresh: true })` 로 쓴다.

**다만 하한이 있다.** 마지막 GBIS 호출로부터 `ARRIVAL_POLL_MIN_MS`(20초)가 지나지 않았으면 캐시를
그대로 돌려준다. 모델이 같은 발화를 여러 번 인식하거나 사용자가 연달아 물으면 `refresh` 가
남발되는데, 그때마다 GBIS 를 부르면 호출량이 그대로 늘어난다. 20초 안에서는 값이 거의 바뀌지
않으므로 캐시를 돌려줘도 사용자 경험에 차이가 없다.

**하한 기준은 `lastAttemptAt`(마지막 호출 시각)이고, 성공·실패 모두 갱신한다.** 낡은 정도를 재는
`fetchedAt`(마지막 성공 시각)과 다른 값이다.

두 시각을 하나로 쓰면 안 된다. 하한을 `fetchedAt` 으로 재면, 실패가 이어지는 동안 그 값이 과거에
머물러 하한이 늘 지나 있는 것으로 보인다. 그러면 `refresh` 가 올 때마다 GBIS 를 다시 두드려
호출이 제한 없이 늘어난다. GBIS 가 죽어 있을 때 호출을 가장 많이 하게 되는 셈이다.

| 시각 | 성공 시 | 실패 시 | 쓰이는 곳 |
|---|---|---|---|
| `fetchedAt` | 갱신 | **유지** | 낡은 정도(`maxStaleMs`) 판정 |
| `lastAttemptAt` | 갱신 | 갱신 | 강제 재조회 하한 |

하한은 캐시 항목에 실려 있으므로 항목이 지워지면 함께 사라진다. 만료 정리는 `refreshAfter` 기준이라
방금 실패한 항목은 걸리지 않는다. 다만 항목 수가 `maxEntries` 를 넘겨 오래된 순서로 정리할 때는
최근 항목도 지워질 수 있고, 그때 해당 대상의 하한은 한 번 초기화된다. 메모리 상한을 지키는 쪽을
택한 결과다.

> 앱·모델 쪽 캐시(이전 안내 문장 재사용)와는 다른 층이다. "버스 놓쳤어요"에서는 모델·앱이 보관한
> 이전 안내를 재사용하지 않고 상태 API 를 새로 호출해야 하고, 그 호출이 실제로 GBIS 까지 갈지는
> 이 하한이 정한다.
