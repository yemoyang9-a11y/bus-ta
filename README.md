...
- 기본 검색은 `DIRECT_BUS`. `ROUTE_SEARCH_SCOPE=MULTIMODAL`을 명시하면 버스 환승·버스와 지하철 혼합 경로를 안내 전용 후보로 포함한다. 지하철 단독 경로와 환승 운행 추적·하차벨은 지원하지 않는다.
...
아래 운행 흐름은 직행버스 후보에 적용한다. 다중교통 후보는 `tripSupported=false`이며 전체 여정은 `segments`로 안내한다. 호환용 최상위 정류장·`stationList`는 첫 번째 버스 구간만 뜻한다.
