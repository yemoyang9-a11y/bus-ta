...
## 0-B. Task 24 완료 (2026-08-20, 효린 로컬 세션)

**0-A.3절이 제안한 1단계 방식("바로 다음 정류장 대조")은 실측으로 기각됐다.** GBIS
`getBusRouteStationListv2`(경유정류소목록조회)로 205(233000281)·200(233000268) 노선
전체를 실제로 받아보니, 수원대학교(233000575) 회차 전/후 두 occurrence의 **바로 다음
정류장이 완전히 동일**했다(같은 도로를 그대로 되짚어 오는 구조). 그래서 대신 **목적지가
노선 전체 순서 어디 있는지**로 방향을 가르는 방식을 채택했다 — 목적지보다 앞서면서 가장
가까운 보딩역 occurrence를 사용자 방향으로 확정한다. 서로 다른 3개 노선(205·200·병점역후문의
55번, 왕복 총 6개 시나리오)으로 실측 검증했다.

**부수적으로 확인한 것**: `GBIS_SERVICE_KEY`는 이미 `busrouteservice`에도 접근 권한이
있었다. 최초 시도에서 난 `NO_OPENAPI_SERVICE_ERROR`는 활용신청 문제가 아니라 엔드포인트
...
**남은 위험(미착수)**: 정류장 이름이 시스템 간 다를 때의 좌표 폴백(100m 이내) 경로는
